...
public class OrderController {
    ...
    public ResponseVO groupTick(@RequestBody OrderEntity order) {
        int count;
        if (orderDao.countTicket(order.getType())!=null){
            count = orderDao.countTicket(order.getType());
        }else {
            count = 0;
        }
        List<Integer> orderEntities = new ArrayList<>();
        if (orderDao.countGroupTime(order.getType()) != null) {
            orderEntities = orderDao.countGroupTime(order.getType());
        } else {
            orderEntities.add(1);
        }
